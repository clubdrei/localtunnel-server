...
class Client extends EventEmitter {
    constructor(options) {
        ...
        agent.on('online', () => {
            console.debug(`[Client.js][${this.id}] Client online`);
            clearTimeout(this.graceTimeout);
        });

        agent.on('offline', () => {
            console.debug(`[Client.js][${this.id}] Client offline`);
    ...
    handleRequest(req, res) {
        console.debug(`[Client.js][${this.id}] Request for ${req.url}`);
        const opt = {
            path: req.url,
            agent: this.agent,
            method: req.method,
        ...
        const clientReq = http.request(opt, (clientRes) => {
            console.debug(`[Client.js][${this.id}] Response for ${req.url} - Status code: ${clientRes.statusCode}`);
        ...
        clientReq.once('error', (error) => {
            console.debug(`[Client.js][${this.id}] clientReq error`, error);
    ...
    handleUpgrade(req, socket) {
        console.debug(`[Client.js][${this.id}] Upgrade request for ${req.url}`);
        socket.once('error', (err) => {
            ...
            console.error(`[Client.js][${this.id}]`, err);
        });

        this.agent.createConnection({}, (err, conn) => {
            console.debug(`[Client.js][${this.id}] Upgrade response for ${req.url}`);
