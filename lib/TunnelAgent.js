...
class TunnelAgent extends Agent {
    ...
    _onConnection(socket) {
        ...
        if (this.connectedSockets >= this.maxTcpSockets) {
            console.debug('no more sockets allowed');
            socket.destroy();
            return false;
        }

        socket.once('close', (hadError) => {
            console.debug('closed socket (error: %s)', hadError);
            ...
            console.debug('connected sockets: %s', this.connectedSockets);
        ...
        this.connectedSockets += 1;
        console.debug('new connection from: %s:%s', socket.address().address, socket.address().port);
    ...
    createConnection(options, cb) {
        ...
        if (!sock) {
            ...
            console.debug('waiting connected: %s', this.connectedSockets);
            console.debug('waiting available: %s', this.availableSockets.length);
            return;
        }
