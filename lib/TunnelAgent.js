...
class TunnelAgent extends Agent {
        ...
        return new Promise((resolve) => {
            getPort({ port: this.client_min_port_range, stopPort: this.client_max_port_range }, (err,port) => {
              server.listen(port,() => {
                  ...
                  console.debug(`TCP server listening on port: ${port}`);
    ...
    _onClose() {
        this.closed = true;
        console.debug('Closed TCP socket');
    ...
    _onConnection(socket) {
        ...
        if (this.connectedSockets >= this.maxTcpSockets) {
            console.error('No more sockets allowed', this.connectedSockets, this.maxTcpSockets);
            socket.destroy();
            return false;
        }

        socket.once('close', (hadError) => {
            console.debug(`Closed socket (error: ${hadError})`);
            ...
            console.debug(`Connected sockets: ${this.connectedSockets}`);
            if (this.connectedSockets <= 0) {
                console.debug('All sockets disconnected');
                this.emit('offline');
            }
        });
        ...
        this.connectedSockets += 1;
        console.debug(`New connection from: ${socket.address().address}:${socket.address().port}`);

        // if there are queued callbacks, give this socket now and don't queue into available
        const fn = this.waitingCreateConn.shift();
        if (fn) {
            console.debug('Giving socket to queued conn request');
            setTimeout(() => {
                fn(null, socket);
            }, 0);
            return;
    ...
    createConnection(options, cb) {
        ...
        if (!sock) {
            ...
            console.debug(`Waiting connected: ${this.connectedSockets}`);
            console.debug(`waiting available: ${this.availableSockets.length}`);
            return;
        }

        console.debug('Socket given');
        cb(null, sock);
    }
